...
create table if not exists users.deed (
	...
	ext         jsonb,
	age			bigint,
	hide		boolean default false,	--是否隐藏（默认不隐藏）
	invalid		boolean default false,	--是否已关闭（默认不隐藏）
	gender		integer,
	created 	timestamp DEFAULT current_timestamp,
	modified 	timestamp DEFAULT current_timestamp
);
...
CREATE TRIGGER "users_wxtrade_modified" BEFORE UPDATE ON "users"."wxtrade"
FOR EACH ROW
EXECUTE PROCEDURE "update_modified"();

ALTER TABLE users.wxtrade OWNER to pguser;
