...
async function checkLoginAtApp(ctx: Router.IRouterContext, next: () => Promise<any>) {
    let token = ctx.cookies.get("token")
    if (!token) {
        token = (ctx as any).headers.token
    }
    try {
        // TODO login
        let res = await RedisLogin.getLoginAsyncAtApp(token)
        if (res.info) {
            (ctx as any).request.loginInfo = res.info
            return next()
        }
...
export class RouterWrap {
    ...
    public loginHandleAtApp(method: string, path: any, f: Router.IMiddleware) {
        this.register(method, f, path, checkLoginAtApp)
    }
